...
export default function FAQS() {
  ...
  const checkOpenAccordion = (indexElement:number,dataUpdate:string[]) => {
    if (accordionBody.current[indexElement]?.classList.contains("hidden")){
      accordionBody.current[indexElement]!.className = "bg-Blond100 rounded-b-lg"
      ...
      accordionBody.current[indexElement]!.className = "hidden bg-Blond100 rounded-b-lg"
  ...
  return (
    <div className='container max-w-7xl mx-auto mt-4 md:mt-12'>
      ...
      <div className="hidden fixed top-0 left-0 right-0 bottom-0 z-40" ref={cardAskQuestion}>
        <div className="max-w-[594px] min-h-[320px] bg-Blond20 translate-x-[75%] translate-y-[75%] shadow-2xl rounded-lg">
          ...
          <div className="w-full flex justify-center mt-6">
            <textarea name="" id="" cols={10} rows={5} placeholder="Ask us anything" className="border border-Neutral02 bg-transparent rounded-lg w-[530px] focus:outline-none focus:ring-0 focus:border-Neutral02" />
          </div>

          <div className="w-full mt-5 text-right">
            <button type="button" className="py-2 px-6 text-Falu100 text-xl font-medium border border-Falu100 rounded mr-4">
              Discard
            </button>
...
const ListFAQS = ({refButton , refBody, onclick , indexElement , dataUpdate}:ListFAQSProp) => {
  return (
    <div className="mb-3">
      ...
      <div ref={(el) => refBody.current[indexElement] = el} className="hidden bg-Blond100 rounded-b-lg" >
